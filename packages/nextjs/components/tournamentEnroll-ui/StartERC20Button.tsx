...
export default function StartERC20Button({tournament_id } : {tournament_id: any}) {
  ...
  const { data: deployedContractData, isLoading: deployedContractLoading } = useDeployedContractInfo('TournamentManager');
    const {
      ...
      functionName: 'startERC20Tournament',
      abi: deployedContractData?.abi,
      args: [Number(tournament_id)],
    });
  ...
  return (
    ...
    type="button"
    onClick={handleWrite}>Start</button>
  )
}
