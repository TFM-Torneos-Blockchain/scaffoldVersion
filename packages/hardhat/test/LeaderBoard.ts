...
import type { Signer, ContractTransaction, ContractReceipt } from "ethers";
...
describe("LeaderBoard and MerkleTree", function () {
  ...
  // // console.log(backendLeaderBoard.concatenatedStringBytes);
  ...
  it("It should generate the same Sponge Hash as the one generated by the Backend.", async () => {
    ...
    // const currentTime=await network.
    // provider.send('evm_mine');
    // await network.provider.send('hardhat_setNextBlockTimestamp', [currentTime + 87400]); // 86400 seconds in a day

    let tx: ContractTransaction = await majorHashGame.connect(owner).play(0);
    let tx3: ContractTransaction = await majorHashGame.connect(participant1).play(0);

    const aaa = leaderBoard.filters.ResultCreated(0);
    const mintEvents = await leaderBoard.queryFilter(aaa);
    type PlayerData = {
      player: string;
      score_number: bigint;
    };
    ...
    for (let i = 0; i < mintEvents.length; i++) {
      const player = mintEvents[i].args.player;
      const scoreNumber = mintEvents[i].args.scoreNumber.toBigInt();

      playerData.push({ player, score_number: scoreNumber });
    }

    const backendLeaderBoard = getLeaderboard(0n, playerData);

    // Log the array of player data
    // console.log("SpongeHAsh:", backendLeaderBoard.spongeHash);
